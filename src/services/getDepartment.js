...
export const getDepartment = async (active_outlet, filters = {}, page, per_page) => {
  ...
  try {
    const data = await apiClient().get(`/api/v1/${active_outlet}/sephcocco_${active_outlet}_departments`, {params});
    return data.data;
  } catch (err) {
    console.error(err);
    throw err;
