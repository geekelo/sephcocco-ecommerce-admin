...
export const getProductCategories = async (active_outlet,filters = {}, page = 1, per_page = 20) => {
  try {
    ...
    const data = await apiClient().get(`/api/v1/${active_outlet}/sephcocco_${active_outlet}_product_categories`, {params});
    return data.data;
  } catch (err) {
    console.error(err);
    throw err;
