...
const EditProductModal = ({ isOpen, onClose, product, categories = [] }) => {
  ...
  const handleMainImageChange = (e) => {
    ...
    if (!file.type.match('image.*')) {
      setErrors({ ...errors, mainImage: 'Please upload only image files (PNG, JPG, JPEG)' });
      e.target.value = null;
      return;
    }
  ...
  const handleImageFiles = (files, isMainImage = false) => {
    ...
    if (isMainImage) {
      ...
      if (formData.images.length + files.length > 4) {
        setErrors({ ...errors, images: 'Maximum 4 additional images are allowed' });
        return;
      }
          ...
          <div className="form-content">
                    ...
                    {formData.category.map(cat => (
                      <span className="badge" key={cat}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            const updated = formData.category.filter(c => c !== cat);
            ...
            <div className={`form-group ${errors.mainImage ? 'error' : ''}`}>
              ...
              {formData.mainImage ? (
                ...
                <div
                  className={`main-image-upload-area ${isMainImageDragging ? 'dragging' : ''}`}
              ...
              {formData.images.length === 0 && (
                ...
                >
                  <div className="upload-placeholder">
                    ...
                    <p>Drag and drop up to 4 additional images or click to upload</p>
                  </div>
                </div>
              )}
